...
struct StringtoFrame {
    ...
    StringtoFrame( const char* str ): StringtoFrame( str, strlen( str )){}
//     {
...
//     }
    ...
    //initializes a frame starting with Programm memory string Arduino F() variant
    StringtoFrame( const __FlashStringHelper *str_p ):
        StringtoFrame( reinterpret_cast<PGM_P>( str_p ), (char *) 0 ) {}
    ...
    //add programm memory String to frame, Arduino F() variant
    bool addString( const __FlashStringHelper *str_p ) {
        return addString( reinterpret_cast<PGM_P>( str_p ), (char *) 0);
    }
